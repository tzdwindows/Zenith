...
### 动画系统
- 基于Assimp的模型加载器，支持glTF、GLB、FBX等格式
- 骨骼动画系统和关键帧插值
- 动画状态机和混合支持
- GPU加速的蒙皮计算
- 高效的动画采样和更新
...
if (!model.getAllAnimations().isEmpty()) {
    String firstAnimName = model.getAllAnimations().keySet().iterator().next();
    animator.play(model.getAnimation(firstAnimName));
...
| LWJGL Assimp | 3D模型加载和动画 | BSD-3-Clause |
